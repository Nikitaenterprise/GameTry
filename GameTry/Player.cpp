...
{
	obj = lev.GetAllObjects();//инициализируем.получаем все объекты дл€ взаимодействи€ персонажа с картой	
	playerScore = 0;
	state = stay;
	isSelect = false;
	if (name == "Player1")
	{
		sprite.setTextureRect(sf::IntRect(int(4), int(19), w, h)); //«адаем спрайту один пр€моугольник дл€ вывода одного льва, а не кучи львов сразу. IntRect - приведение типов
	}
}
...
void Player::Control()
{
	if ((sf::Keyboard::isKeyPressed(sf::Keyboard::Left) || (sf::Keyboard::isKeyPressed(sf::Keyboard::A))))
	{
		...
		//if (CurrentFrame > 3) CurrentFrame -= 3; // если пришли к третьему кадру - откидываемс€ назад.
		//player1.sprite.setTextureRect(sf::IntRect(96 * int(CurrentFrame), 96, 96, 96)); //проходимс€ по координатам ’. получаетс€ начинаем рисование с координаты ’ равной 0,96,96*2, и оп€ть 0
	}
